...
NoteOnEvent and NoteOffEvent renamed to NoteOn and NoteOff. The old names
will still work for a while.

The Event boolean methods like meta? and note? have been removed. Use the
event classes themselves (for example, MIDI::MetaEvent === my_event or
my_event.kind_of?(MIDI::MetaEvent)).

Introduced Adam Murray's stable sorting code for
Track#recalc_delta_from_times. See
...
Aliased Track#sort to Track#recalc_delta_from_times, since all sort did was
sort the events then call recalc_delta_from_times, and
